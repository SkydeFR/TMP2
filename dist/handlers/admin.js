...
const stringToOptions = require('../util/stringtooptions'); // const {ObjectId} = require('mongoose').Types;
...
router.get('/users/count', (req, res) => {
  ...
  const {
    status
  } = stringToOptions(req.query.options, {
    status: 'all'
  });
  const projection = status === 'all' ? {} : status === 'admin' ? {
    status: 'admin'
  } : {
    status: 'user'
  };
...
router.post('/types/edit', async (req, res) => {
  if (!req.body.nom || !req.body.id) {
    ...
    try {
      const type = await Type.findByIdAndUpdate(req.body.id, {
        nom: req.body.nom
      }).exec();
...
router.post('/users/changeadmin', async (req, res) => {
  if (!req.body.id) {
    ...
    try {
      ...
      const admin = (_req$body$admin = req.body.admin) !== null && _req$body$admin !== void 0 ? _req$body$admin : true;
    ...
    if (!debut) {
      ...
      Destination.findOne({
        ...
        if (lieu) {
          ...
          if (modeTemps && _.filter(req.body.types, element => !isValid(element)).length !== 0) {
            ...
            try {
              ...
              if (listeTypes.length < req.body.types.length) {
                ...
                destination.save().then(async destination => {
          ...
          destination.save().then(destination => {
            res.status(200).json({
              code: 'success',
              destination
            });
