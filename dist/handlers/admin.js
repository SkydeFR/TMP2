...
const router = express.Router(); // const {ObjectId} = require('mongoose').Types;
...
function stringToOptions(optionsEnter, defaultOptions) {
  ...
  const optionsString = (_optionsEnter = optionsEnter) !== null && _optionsEnter !== void 0 ? _optionsEnter : '{}';
  ...
  if (!req.body.nom) {
    ...
    Type.find({
      ...
      if (type) {
        ...
        type.save().then(type => res.status(200).json({
          code: 'success',
          type
        })).catch(err => {
          console.error(err);
